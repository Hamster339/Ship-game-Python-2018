...
class Enemy():
    ...
    def update(self):
        ...
        def Direction_calc():
            ...
            self.range = math.hypot(self.x - game.player.x, self.y - game.player.y)
            if self.range < 275:
                self.circleing = True
        ...
        def Position(): #calculates variables needed to determin position
        ...
        def Fire():
            if self.range < 250 and self.circleing == True:
                if self.relitive_angle > 80 and self.relitive_angle < 100 and self.reloadingL == False:
                    self.left_fireing = 4
                    self.cannonl1 = Cannon(30,"l","e")
                    self.cannonl2 = Cannon(16,"l","e")
                    self.cannonl3 = Cannon(2,"l","e")
                    self.cannonl4 = Cannon(-5,"l","e")
                    ...
                    self.cannonr1 = Cannon(30,"r","e")
                    self.cannonr2 = Cannon(16,"r","e")
                    self.cannonr3 = Cannon(2,"r","e")
                    self.cannonr4 = Cannon(-5,"r","e")
...
class Cannon():
    def __init__(self,offset,direction,source):
        ...
        if source == "p":
            if direction == "l":
                self.angle = game.player.angle - 90
            elif direction == "r":
                self.angle = game.player.angle + 90
        else:
            if direction == "l":
        ...
        if source == "p":
            self.x = int(game.player.x + game.player.ship[0].get_rect().centerx)
            self.y = int(game.player.y + game.player.ship[0].get_rect().centery)
        else:
            self.x = int(game.enemy.x + game.enemy.ship[0].get_rect().centerx)
    ...
    def collitions(self):
            ...
            elif self.distance >= self.range: #if cannon ball gose past max range
...
class Main():
    def __init__(self):
        ...
        self.play_rect = pygame.Rect((39,466),(300,100))
        ...
        self.resume_rect = pygame.Rect((336,231),(300,100))
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if self.player.reloadingL == False and self.player.speed_change != "knocked":
                            ...
                            self.player.cannonl1 = Cannon(30,"l","p")
                            self.player.cannonl2 = Cannon(16,"l","p")
